...
public class Tool {
  ...
  public static synchronized boolean isImage(String file) {
    ...
    if (file != null) {
      ...
      if (file.endsWith("jpg") || file.endsWith("jpeg") || file.endsWith(".jpeg") || file.endsWith(".png") || file.endsWith("gif")) {
        sw = true;
      }
    }
    return sw;
  ...
  public static synchronized String preview(String upDir, String _src, int width,  int height) {
    ...
    // 이미지 파일인지 검사
    if (name.endsWith("jpg") || name.endsWith("jpg") || name.endsWith("bmp") || name.endsWith("png")
        || name.endsWith("gif")) {
      try {
