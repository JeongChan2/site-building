...
DROP TABLE contents_stock;

CREATE TABLE contents_stock(
        contents_num                            NUMBER(10)         NOT NULL         PRIMARY KEY,
        admin_num                              NUMBER(10)     NOT NULL , -- FK
        ...
        FOREIGN KEY (admin_num) REFERENCES admin_stock (admin_num),
        FOREIGN KEY (cate_id) REFERENCES category (cate_id)
);

COMMENT ON TABLE contents_stock is '컨텐츠 - 순례길';
COMMENT ON COLUMN contents_stock.contents_num is '컨텐츠 번호';
COMMENT ON COLUMN contents_stock.admin_num is '관리자 번호';
COMMENT ON COLUMN contents_stock.cate_id is '카테고리 번호';
COMMENT ON COLUMN contents_stock.title is '제목';
COMMENT ON COLUMN contents_stock.content is '내용';
COMMENT ON COLUMN contents_stock.recom is '추천수';
COMMENT ON COLUMN contents_stock.cnt is '조회수';
COMMENT ON COLUMN contents_stock.replycnt is '댓글수';
COMMENT ON COLUMN contents_stock.passwd is '패스워드';
COMMENT ON COLUMN contents_stock.word is '검색어';
COMMENT ON COLUMN contents_stock.rdate is '등록일';
COMMENT ON COLUMN contents_stock.file1 is '메인 이미지';
COMMENT ON COLUMN contents_stock.file1saved is '실제 저장된 메인 이미지';
COMMENT ON COLUMN contents_stock.thumb1 is '메인 이미지 Preview';
COMMENT ON COLUMN contents_stock.size1 is '메인 이미지 크기';
COMMENT ON COLUMN contents_stock.price is '정가';
COMMENT ON COLUMN contents_stock.dc is '할인률';
COMMENT ON COLUMN contents_stock.saleprice is '판매가';
COMMENT ON COLUMN contents_stock.point is '포인트';
COMMENT ON COLUMN contents_stock.salecnt is '수량';
COMMENT ON COLUMN contents_stock.map is '지도';
COMMENT ON COLUMN contents_stock.youtube is 'Youtube 영상';

DROP SEQUENCE contents_stock_seq;

CREATE SEQUENCE contents_stock_seq
  START WITH 1                -- 시작 번호
  INCREMENT BY 1            -- 증가값
