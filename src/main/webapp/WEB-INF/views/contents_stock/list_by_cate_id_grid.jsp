...
<div style="text-align: right; clear: both;">  
  <form name='frm' id='frm' method='get' action='./list_by_cate_id_grid.do'>
    ...
    <c:if test="${param.word.length() > 0 }"> <%-- 검색 상태라면 '검색 취소' 버튼을 출력 --%>
      <button type='button' class='btn btn-info btn-sm' 
                  onclick="location.href='./list_by_cate_id_grid.do?cate_id=${param.cate_id}&word='">검색 취소</button>  
    </c:if>    
  </form>
</div>
...
<div class="menu_line"></div>
  ...
  <div style='width: 100%;'> <%-- 갤러리 Layout 시작 --%>
    <c:forEach var="contents_stockVO" items="${list }" varStatus="status">
      <c:set var="title" value="${contents_stockVO.title }" />
      <c:set var="content" value="${contents_stockVO.content }" />
      <c:set var="cate_id" value="${contents_stockVO.cate_id }" />
      <c:set var="contents_num" value="${contents_stockVO.contents_num }" />
      <c:set var="thumb1" value="${contents_stockVO.thumb1 }" />
      ...
      <div onclick="location.href='./read.do?contents_num=${contents_num}&word=${param.word }&now_page=${param.now_page == null ? 1 : param.now_page }'"
       style='width: 19%; height: 200px; float: left; margin: 0.5%; padding: 0.5%; background-color: #EEEFFF; text-align: left; cursor: pointer;'>
        ...
        <c:choose> 
          <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}"> <%-- 이미지인지 검사 --%>
            ...
            <img src="/contents/storage/${thumb1 }" style="width: 100%; height: 120px;">
          </c:when>
          <c:otherwise> <!-- 이미지가 없는 경우 기본 이미지 출력: /static/contents/images/none1.png -->
            <IMG src="/contents_stock/images/none1.png" style="width: 100%; height: 120px;">
          </c:otherwise>
        </c:choose>
        ${title }
